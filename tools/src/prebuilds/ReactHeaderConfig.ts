/**
 * Static podspec exception configuration for React Native header collection.
 *
 * Ported from react-native's `scripts/ios-prebuild/headers-config.js`.
 * These entries describe complex podspecs where simple regex-based parsing
 * of header_dir and source_files is insufficient. Each entry maps a podspec
 * path (relative to the RN root) to its header configuration.
 */

export interface PodSpecConfiguration {
  name: string;
  headerPatterns: string[];
  headerDir?: string;
  excludePatterns?: string[];
  subSpecs?: PodSpecConfiguration[];
  preservePaths?: string[];
  disabled?: boolean;
}

export const PodspecExceptions: Record<string, PodSpecConfiguration | { disabled: true }> = {
