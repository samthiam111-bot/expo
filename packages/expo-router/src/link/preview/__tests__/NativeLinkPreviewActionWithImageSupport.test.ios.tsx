...
import { NativeLinkPreviewActionWithImageSupport } from '../NativeLinkPreviewActionWithImageSupport';
...
const { NativeLinkPreviewAction } = jest.requireMock('../native') as typeof import('../native');
...
describe(NativeLinkPreviewActionWithImageSupport, () => {
  const baseProps = {
    identifier: 'test-id',
    title: 'Test',
    onSelected: jest.fn(),
  ...
  it('renders NativeLinkPreviewAction directly when no xcassetName', () => {
    render(<NativeLinkPreviewActionWithImageSupport {...baseProps} />);
  ...
  it('calls useImage with xcassetName when provided', () => {
    render(<NativeLinkPreviewActionWithImageSupport {...baseProps} xcassetName="my-icon" />);

    expect(mockUseImage).toHaveBeenCalledWith({ uri: 'my-icon' }, {}, ['my-icon']);
  });

  it('passes resolved image to NativeLinkPreviewAction when xcassetName is provided', () => {
    ...
    expect(MockedNativeLinkPreviewAction.mock.calls[0][0].image).toBe(fakeImageRef);
  });
  ...
  it('prefers explicit image prop over xcasset image', () => {
    ...
    expect(MockedNativeLinkPreviewAction.mock.calls[0][0].image).toBe(explicitImage);
  });
  ...
  it('passes null image when useImage has not resolved yet', () => {
    ...
    // undefined ?? null = null
    expect(MockedNativeLinkPreviewAction.mock.calls[0][0].image).toBeNull();
  });
  ...
  describe('imageSource support', () => {
    it('calls useImage with imageSource when provided', () => {
      ...
      render(<NativeLinkPreviewActionWithImageSupport {...baseProps} imageSource={imageSource} />);

      expect(mockUseImage.mock.calls[0][0]).toEqual(imageSource);
    });

    it('passes resolved image to NativeLinkPreviewAction when imageSource is provided', () => {
      const fakeImageRef = { __expo_shared_object_id__: 42 } as unknown as ImageRef;
      mockUseImage.mockReturnValue(fakeImageRef);
