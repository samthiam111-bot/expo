...
exports.sendCliMessageAsync = sendCliMessageAsync;
const CliExtensionUtils_js_1 = require("./CliExtensionUtils.js");
...
async function sendCliMessageAsync(message, pluginName, apps, timeoutMs = DEFAULT_TIMEOUT_MS) {
    ...
    if (apps.length === 0) {
        return Promise.reject(new CliExtensionUtils_js_1.SendMessageError('No apps provided to send the message to.', apps[0]));
    }
    ...
    if (apps.some((app) => new URL(app.webSocketDebuggerUrl).host !== new URL(apps[0].webSocketDebuggerUrl).host)) {
        return Promise.reject(new CliExtensionUtils_js_1.SendMessageError('All apps should share the same WebSocket URL hostname to send messages.' +
    ...
    return new Promise((resolve, reject) => {
        ...
        const timeoutHandler = setTimeout(() => {
            ...
            if (Object.values(results).every((result) => result === null)) {
                ...
                reject(new CliExtensionUtils_js_1.SendMessageError(errorMessage, apps.find((a) => results[a.id] === null)));
            }
